...
public class TestData {
    ...
    // 初始化Data的solution
    public static void initSolution(int n) {
        TASK_NUM = n;
    ...
    public static void initForAllocate(List<Set<Integer>> allocationNo, int taskNum) throws IOException {
        // 初始化AGC位置
    ...
    public static void genTasks(List<Set<Integer>> allocationNo) {
        if (taskMap == null) {
            taskMap = new HashMap<>();
        }
        if (taskMap.size() > 0) {
        ...
        for (int i = 0; i < allocationNo.size(); i++) {
            Set<Integer> candidates = allocationNo.get(i);
            for (int candidate : candidates) {
                Location start = startLocation;
                Location end = stockLocationMap.get(candidate);
                Task task = new Task(index, 0, start, end, 1);
